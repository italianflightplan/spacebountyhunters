...
PLUGIN.helperFuncs["getDamage"] = function(self, weapon)
	...
	if(char) then
		...
		if(table.IsEmpty(totalDam)) then
			totalDam[1] = {
				dmg = char:getAttrib("strength", 0) * 0.1 + self:getBuffAttribute("dmg"),
				dmgT = "Standard",
				weap = "Hands",
				accuracy = self:getAccuracy()
			}
...
PLUGIN.helperFuncs["getRes"] = function(self)
	...
	res["dmg"] = (res["dmg"] or 0) + (char:getAttrib("toughness", 0) * 0.25)
	res["effect"] = (res["effect"] or 0) + (char:getAttrib("toughness", 0) * 0.25) + (char:getAttrib("m", 0) * 0.25)
...
PLUGIN.helperFuncs["getArmor"] = function(self)
	...
	if(char) then
		...
		armor = armor + (char:getAttrib("toughness", 0) * 3)
...
PLUGIN.helperFuncs["getEvasion"] = function(self)
	...
	if(char) then
		evasion = evasion + (char:getAttrib("dexterity", 0) * 0.25)
...
PLUGIN.helperFuncs["getAccuracy"] = function(self)
	...
	if(char) then
		accuracy = accuracy + (char:getAttrib("strength", 0) * 2)
		
		accuracy = accuracy + (char:getAttrib("agility", 0) * 2)
...
PLUGIN.helperFuncs["getMagic"] = function(self)
	...
	local intelligence = char:getAttrib(mindstat*0.5 + intelligencestat*0.25 + arcanestat*0.25 + luckstat*0.1, 0)
